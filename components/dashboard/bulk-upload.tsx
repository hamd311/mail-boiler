...
export function BulkUpload({ onResults }: BulkUploadProps) {
  ...
  const isButtonDisabled = loading || hasInsufficientCredits;

  const getTooltipMessage = () => {
    if (hasInsufficientCredits) {
      return `Insufficient credits. You need ${emails.length} credits but only have ${user?.subscription.remaining_credits} available.`;
    }
    return "";
  };
        ...
        <div className="space-y-6 p-8">
                    ...
                    <TooltipTrigger asChild>
                      <span tabIndex={0} className="block">
                        <Button
                          ...
                          className="h-12 w-full bg-gradient-to-r from-[#10b981] via-[#06b6d4] to-[#3b82f6] text-white transition-all duration-300 hover:opacity-90 hover:shadow-lg hover:shadow-[#10b981]/25 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {loading ? (
                            <>
          ...
          <Alert className="border-blue-200 bg-gradient-to-r from-blue-50 to-cyan-50 dark:border-blue-900 dark:from-blue-950/30 dark:to-cyan-950/30">
            ...
            <AlertDescription className="text-blue-800 dark:text-blue-200">
              ...
              {emails.length > 0 && (
                <>
                  This operation will use <strong>{emails.length}</strong>{" "}
                  credit
                  {emails.length !== 1 ? "s" : ""}.
                </>
              )}
